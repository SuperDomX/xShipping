...
/*		
		...
		$data = 'API=RateV4&XML=<RateV4Request USERID="'.$this->usps_user.'" PASSWORD="'.$this->usps_password.'">';
*/
		...
		$data = "API=RateV4&XML=<RateV4Request USERID=\"".$this->usps_user."\"><Package ID=\"1ST\"><Service>ALL</Service><ZipOrigination>".$this->from_zip."</ZipOrigination><ZipDestination>".$this->to_zip."</ZipDestination><Pounds>".number_format(($this->weight_unit != 'lb' ? convert_weight($this->weight,$this->weight_unit,'lb') : $this->weight), 1, '.', '')."</Pounds><Ounces>0</Ounces><Container/><Size>REGULAR</Size><Machinable>TRUE</Machinable></Package></RateV4Request>";
		...
		// Debug
		if($this->debug == true) {
			print "<xmp>".print_r($params)."</xmp><br />";
			print "<xmp>".$results."</xmp><br />";
		}
/*		
		// Match Rate(s)
...
*/
		foreach($params['RATEV4RESPONSE']['1ST'] as $key => $value) {
			if(is_numeric($key)) {
